  ...
  <div className="flex h-full min-h-0 flex-col border-r">
    ...
    <div className="flex-1 overflow-y-auto">
      ...
      {conversations?.length === 0 && (
        <p className="text-muted-foreground p-4 text-sm">Nenhuma conversa por aqui.</p>
      )}
      ...
      {conversations?.map((c) => (
        ...
        >
          <Avatar className="size-10 shrink-0">
            ...
            <AvatarFallback>{titulo(c).slice(0, 2).toUpperCase()}</AvatarFallback>
          </Avatar>

          <div className="min-w-0 flex-1">
            <div className="flex items-baseline justify-between gap-2">
              <span className="truncate text-sm font-medium">{titulo(c)}</span>
              {c.lastMessageAt && (
                <span className="text-muted-foreground shrink-0 text-xs">
                  {formatDistanceToNowStrict(parseISO(c.lastMessageAt), { locale: ptBR })}
                </span>
              )}
            </div>
            <div className="flex items-center justify-between gap-2">
              ...
              {c.unreadCount > 0 && (
                <Badge className="h-5 min-w-5 shrink-0 justify-center px-1.5">{c.unreadCount}</Badge>
              )}
            </div>
            {/* sem lead vinculado é o caso que o corretor precisa agir: virar lead ou escolher */}
            {!c.client && (
              <span className="text-muted-foreground text-[11px] italic">sem lead vinculado</span>
            )}
          </div>
        </button>
      ))}
