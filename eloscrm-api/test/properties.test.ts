...
beforeAll(async () => {
  ...
  ({ cookie: cookieB, orgId: orgBId } = await signUpWithOrg(app, `properties-b-${stamp}@eloscrm.test`, `properties-b-${stamp}`));
});

afterAll(async () => {
  await prisma.property.deleteMany({ where: { organizationId: { in: [orgId, orgBId] } } });
  await prisma.organization.deleteMany({ where: { slug: { startsWith: `properties-` } } });
