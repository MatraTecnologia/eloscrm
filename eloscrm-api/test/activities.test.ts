...
beforeAll(async () => {
  ...
  ({ cookie: cookieB, orgId: orgBId } = await signUpWithOrg(app, `activities-b-${stamp}@eloscrm.test`, `activities-b-${stamp}`));
});
...
afterAll(async () => {
  ...
  await prisma.organization.deleteMany({ where: { slug: { startsWith: `activities-` } } });
